This document presents two demonstration of the software suite CeleST ("C. elegans Swim Test").

Our code is meant to be run under Matlab 2011 and above, by setting the Matlab path to the corresponding demo directory containing the file CeleST.m . The software is run by entering the command "CeleST" into Matlab Command Window. The directory containing the matlab source code is meant to be write-accessible by the program, therefore we recommend copying the entire directory of this demonstration into a writable folder (such as the user's desktop, or a temporary folder).
